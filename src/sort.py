    ...
    def update(self, dets, img_size, root_dic, addtional_attribute_list, predict_num):
        ...
        for trk in reversed(self.trackers):
            ...
            if (trk.time_since_update < 1) and (trk.hit_streak >= self.min_hits or self.frame_count <= self.min_hits):
                # print("d = ", d)
                # print("trk.face_addtional_attribute = ", trk.face_addtional_attribute)
                ret.append(np.concatenate((d, [trk.id + 1], [trk.face_addtional_attribute[0][5], trk.face_addtional_attribute[0][6]])).reshape(1, -1))  # +1 as MOT benchmark requires positive
            i -= 1
